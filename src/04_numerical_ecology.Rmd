...
There are a total of `r controls %>% summarise(sum = sum(n)) %>% pull()`
...
set.seed(seed)
matrix1 <- rrarefy(occurrences_t, smallest_sample_size)
for (i in 2:n_subsamplings) {
    matrix1 <- matrix1 + rrarefy(occurrences_t, smallest_sample_size)
}
...
As expected, there are now `r smallest_sample_size` reads in all samples.
