...
. %>%
    ...
    filter(! (sample %in% dubious_samples) &
           ...
           taxo_method == "bracken" &
           reads > 0.0) %>%
    count(soil_code, read_pass, taxonomy,
          wt = reads, name = "reads") -> reduce_and_pool_resequenced_samples
...
big_table %>%
    count(soil_code, read_pass, wt = reads, name = "reads") %>%
    arrange(reads) -> sample_sizes

sample_sizes %>% head()
sample_sizes %>% tail()

sample_sizes %>% head(n = 1) %>% pull(reads) -> smallest_sample_size

```

The difference between the largest and the smallest samples is larger
than expected. That should be investigated.
...
### TODO rarefaction (random subsampling)
...
As expected, there are now ... reads in all samples.
